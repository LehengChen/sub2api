...
- Upstream tag: `v0.1.175`
- Upstream peeled commit: `93c32fa1a2450351561abc46156d2e28cb5f74ca`
- Upstream capacity recovery: `8f7b0a314de816daabb5b761db3025cb10c3eca9` (PR #5398)
- Baseline status: v0.1.175 is merged only on the isolated integration branch.
...
- `backend/cmd/server/VERSION` is normalized from the tag's stale `0.1.173` to
  `0.1.175`; release builds should still inject the immutable candidate version.
- FZ-010 is carried as a local reimplementation because the upstream capacity
  recovery remains broader than the frozen contract: v0.1.175 stages generic
...
last_reviewed_against: 93c32fa1a2450351561abc46156d2e28cb5f74ca
```

- Exit and quality probes remain HTTPS-only. Upstream v0.1.175 still uses the
...
- Recalculated on the v0.1.175 module graphs: `golang.org/x/image` is 0.43.0,
...
- Non-transactional migrations 175a and 190 now drop an invalid concurrent
  index before retrying, while v0.1.175 migration behavior is preserved.
...
- Externally managed deployments expose read-only version health but disable
  in-application update, rollback, and restart controls. The v0.1.175 Wire
...
- Claude, OpenAI, Grok, Gemini, and Antigravity OAuth state is injected through
  the fail-closed Redis session store. The v0.1.175 Grok password/SSO and token
...
- Explicit active/api/worker/standby/migrator roles, Redis worker lease and
  ...
  carried. The v0.1.175 Channel Monitor runner and V2 aggregator are also
...
- Every redirect hop revalidates scheme, host allowlist, userinfo, port, and
  private DNS resolution. v0.1.175 transport behavior is preserved.
...
- The v0.1.175 tree carries the FZ-005 lifecycle handlers, but its embedded
