...
func TestApplyMigrationsFS_NonTransactionalMigration_OpsSystemLogsHostIndexDropsInvalidIndexBeforeRetry(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	fsys := fstest.MapFS{
		opsSystemLogsHostIndexMigration: &fstest.MapFile{
			Data: []byte("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ops_system_logs_host_created_at ON ops_system_logs (host, created_at DESC);"),
		},
	}
...
func TestApplyMigrationsFS_NonTransactionalMigration_UsersEmailAliasIndexDropsInvalidIndexBeforeRetry(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	fsys := fstest.MapFS{
		usersEmailAliasIndexMigration: &fstest.MapFile{
			Data: []byte("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_email_dot_stripped ON users ((REPLACE(LOWER(TRIM(email)), '.', '')) text_pattern_ops) WHERE deleted_at IS NULL;"),
		},
	}
