...
func SetupRouter(
	...
	// 应用中间件
	r.Use(healthService.DrainGuard())
	r.Use(healthService.TrackRequests())
	...
	// 注册路由
	registerRoutes(r, handlers, jwtAuth, optionalJWTAuth, adminAuth, apiKeyAuth, auditLog, stepUpAuth, apiKeyService, subscriptionService, opsService, settingService, compositeResolver, cfg, redisClient, healthService)
...
func registerRoutes(
	...
	// 通用路由（健康检查、状态等）
	routes.RegisterCommonRoutes(r, healthService.LivenessHandler, healthService.ReadinessHandler)
